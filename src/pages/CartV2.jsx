...
import { beginStudioCartEdit } from "@/lib/customStudioDraftBridge";
  ...
  return (
    <div className={`${frameClass} relative overflow-hidden border border-black/10`}>
      ...
      {customPreview && (
        ...
        >
          <button type="button" onClick={() => adjustZoom(-0.1)} disabled={zoom <= 0.8} className="grid h-9 w-9 place-items-center text-[#52616F] transition hover:bg-black/[.04] disabled:opacity-35" aria-label="Zoom garment preview out"><ZoomOut size={14} /></button>
          <span className="min-w-11 text-center font-mono text-[9px] font-bold tabular-nums text-[#52616F]">{Math.round(zoom * 100)}%</span>
          <button type="button" onClick={() => adjustZoom(0.1)} disabled={zoom >= 1.8} className="grid h-9 w-9 place-items-center text-[#52616F] transition hover:bg-black/[.04] disabled:opacity-35" aria-label="Zoom garment preview in"><ZoomIn size={14} /></button>
          <button type="button" onClick={() => setZoom(1)} className="h-9 border-l border-[#DCE3EA] px-2.5 text-[8px] font-black uppercase tracking-wide text-[#52616F]" aria-label="Fit garment preview to canvas">Fit</button>
        </div>
      )}
    </div>
  );
                  ...
                  <div className="grid gap-4 sm:grid-cols-[180px_minmax(0,1fr)] sm:gap-6">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-5 flex flex-wrap items-center gap-3">
                        ...
                        {canEditCustom && <button type="button" onClick={() => editCustomDesign(item)} className="inline-flex min-h-11 items-center gap-2 border border-black/15 px-4 text-sm font-bold hover:bg-black/5"><Pencil size={15} /> Edit design</button>}
                      ...
                      {canEditCustom && <p className="mt-3 text-xs leading-5 text-muted-foreground">Edit design reopens this saved Custom Studio configuration. After changes, you must approve it again; adding it back replaces this cart item instead of creating a duplicate.</p>}
