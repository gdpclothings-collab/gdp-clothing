...
export default function Cart() {
  ...
  return (
    <main className="bg-[#f6f5f1] text-foreground">
      <div className="mx-auto max-w-[1400px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
        <header className="mb-7 flex flex-wrap items-end justify-between gap-3 border-b border-black/10 pb-6">
          <div>
          ...
          <section aria-label="Cart items" className="space-y-4">
            {items.map((item) => (
              <article key={item.key} className="border border-black/10 bg-white p-4 shadow-[0_8px_30px_rgba(0,0,0,.035)] sm:p-5">
                <div className="grid gap-4 sm:grid-cols-[150px_minmax(0,1fr)] sm:gap-6">
                  <div className="aspect-square overflow-hidden border border-black/10 bg-secondary sm:aspect-[4/5]">
                    {item.isDtf
                  ...
                  <div className="min-w-0">
                    ...
                    {item.isDtf && item.dtfSpec ? (
                      ...
                      <p className="mt-3 text-sm text-muted-foreground">{[item.color, item.size].filter(Boolean).join(" · ")}</p>
                    )}

                    {item.isCustom && !item.isDtf && (
                    ...
                    <div className="mt-5 flex flex-wrap items-center gap-3">
                      ...
                      {item.seasonalDraft && (
                        <Link to={`/custom-studio?product=${encodeURIComponent(item.productId)}&color=${encodeURIComponent(item.color || "")}&size=${encodeURIComponent(item.size || "")}`} state={{ seasonalDraft: item.seasonalDraft, editCartKey: item.key }} className="inline-flex min-h-11 items-center gap-2 border border-black/15 px-4 text-sm font-bold"><Pencil size={15} /> Edit design</Link>
                      )}
...
function Spec({ label, value }) {
  return <div><dt className="text-xs text-muted-foreground">{label}</dt><dd className="mt-0.5 font-mono text-xs font-bold">{value}</dd></div>;
}
