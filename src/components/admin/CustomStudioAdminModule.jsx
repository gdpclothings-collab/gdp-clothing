...
function CustomStudioSettingsPanel({ settings, loading, saving, dirty, onChange, onSave, onIntensityImageChange }) {
  ...
  const editorTools = normalizeEditorTools(settings.editorTools);
  const stickerLibrary = normalizeStickerLibrary(settings.stickerLibrary);

  const updateEditorTool = (key, value) => {
    onChange("editorTools", { ...editorTools, [key]: value });
  };

  const updateSticker = (stickerId, patch) => {
    const next = stickerLibrary.map((item) => item.id === stickerId ? { ...item, ...patch } : item);
    onChange("stickerLibrary", next);
  };

  const uploadStickerArtwork = async (sticker, file) => {
    if (!file || !sticker) return;
    try {
      const url = await adminSettingsApi.uploadStickerAsset(file, sticker.id);
  ...
  return (
    <div className="space-y-5">
      <div className="grid xl:grid-cols-[1fr_.9fr] gap-5">
        <section className="rounded-xl border border-[#dedede] bg-white overflow-hidden">
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="rounded-lg border border-[#d8e0e7] bg-[#f7fafc] p-3">
              <div className="text-xs font-semibold text-[#17324D]">Custom editor tools</div>
              <div className="mt-1 text-[10px] leading-4 text-[#66727d]">Enable or disable customer editing capabilities without changing locked GDP template protection.</div>
              <div className="mt-3 grid sm:grid-cols-2 gap-2">
                <SettingToggle icon={PencilRuler} title="Erase tool" description="Allow erasing parts of customer-uploaded photos." checked={editorTools.erase !== false} onChange={(value) => updateEditorTool("erase", value)} />
                <SettingToggle icon={RefreshCw} title="Restore tool" description="Allow restoring erased photo pixels from the preserved source." checked={editorTools.restore !== false} onChange={(value) => updateEditorTool("restore", value)} />
                <SettingToggle icon={Sparkles} title="Sticker library" description="Allow customers to add editable sticker and graphic layers." checked={editorTools.stickers !== false} onChange={(value) => updateEditorTool("stickers", value)} />
                <SettingToggle icon={PencilRuler} title="Extra text layers" description="Allow multiple movable, resizable and rotatable text layers." checked={editorTools.text !== false} onChange={(value) => updateEditorTool("text", value)} />
                <SettingToggle icon={SlidersHorizontal} title="Own-artwork free stretch" description="Allow customers who upload their own artwork to unlock aspect ratio." checked={editorTools.freeStretch !== false} onChange={(value) => updateEditorTool("freeStretch", value)} />
                <SettingToggle icon={ImageIcon} title="Auto background removal" description="Attempt conservative background cleanup for Photo Bootleg uploads while preserving originals." checked={editorTools.autoBackgroundRemoval !== false} onChange={(value) => updateEditorTool("autoBackgroundRemoval", value)} />
              </div>
            </div>
                ...
                <div className="p-3 space-y-3">
                  <div className="grid sm:grid-cols-2 gap-2">
                    <label className="text-[10px] font-medium text-[#555]">
                      Template name
                      <input value={style.name || ""} onChange={(event) => updateStyleTemplate(style.id, { name: event.target.value })} className="mt-1 h-8 w-full rounded-md border border-[#d4d4d4] bg-white px-2 text-xs" />
                    </label>
                    <label className="text-[10px] font-medium text-[#555]">
                      Description
      ...
      <section className="rounded-xl border border-[#dedede] bg-white overflow-hidden">
        <div className="px-4 py-3 border-b border-[#e8e8e8] flex items-center justify-between gap-3">
          <div>
            <div className="text-sm font-semibold">Sticker library</div>
            <div className="text-xs text-[#777] mt-0.5">Activate, categorize or replace starter stickers with transparent PNG/WEBP artwork.</div>
          </div>
          <button type="button" onClick={onSave} disabled={saving || !dirty} className="h-9 px-3 rounded-lg bg-[#222] text-white text-xs font-semibold inline-flex items-center justify-center gap-2 disabled:opacity-40"><Save size={13}/> Save stickers</button>
        </div>
        <div className="p-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {stickerLibrary.map((sticker) => (
            <div key={sticker.id} className="rounded-xl border border-[#e1e1e1] bg-[#fafafa] p-3">
              <div className="flex items-start gap-3">
                <div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-lg border border-[#ddd] bg-white text-2xl">
                ...
                <div className="min-w-0 flex-1">
                  <input value={sticker.label || ""} onChange={(event) => updateSticker(sticker.id, { label: event.target.value })} className="h-8 w-full rounded-md border border-[#d4d4d4] bg-white px-2 text-xs font-semibold"/>
                  <div className="mt-2 grid grid-cols-2 gap-2">
                    <input value={sticker.category || ""} onChange={(event) => updateSticker(sticker.id, { category: event.target.value })} className="h-8 rounded-md border border-[#d4d4d4] bg-white px-2 text-[10px]" placeholder="Category"/>
                    <label className="flex items-center justify-center gap-1.5 rounded-md border border-[#d4d4d4] bg-white px-2 text-[10px] font-semibold"><input type="checkbox" checked={sticker.enabled !== false} onChange={(event) => updateSticker(sticker.id, { enabled: event.target.checked })}/> Active</label>
                  </div>
                </div>
              </div>
              <div className="mt-3 grid grid-cols-[1fr_auto] gap-2">
                <label className="h-9 rounded-lg bg-[#222] text-white text-[10px] font-semibold inline-flex items-center justify-center gap-1.5 cursor-pointer">
                  <Upload size={12}/> {sticker.assetUrl ? "Replace PNG" : "Upload PNG"}
                  <input type="file" accept="image/png,image/webp" className="hidden" onChange={(event) => uploadStickerArtwork(sticker, event.target.files?.[0])}/>
                </label>
                <button type="button" disabled={!sticker.assetUrl} onClick={() => updateSticker(sticker.id, { assetUrl: "" })} className="h-9 rounded-lg border border-[#d7d7d7] bg-white px-2 text-[10px] font-semibold disabled:opacity-35">Default</button>
              </div>
            </div>
          ))}
        </div>
