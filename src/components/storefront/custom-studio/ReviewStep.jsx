...
export default function ReviewStep({ model }) {
  ...
  const seasonalPreparedReady = designPath !== "seasonal" || Boolean(
    ...
    seasonalPrepared?.cartItemBase?.renderStatus === "locked" &&
    seasonalPrepared?.designPayload?.renderStatus === "locked" &&
    seasonalPrepared?.designPayload?.productionFiles?.front?.path &&
    seasonalPrepared?.designPayload?.customerMockupPath &&
    seasonalPrepared?.designPayload?.lockedHash &&
    seasonalPrepared?.designPayload?.preflight?.status === "passed" &&
  ...
  return (
    <div>
      ...
      {designPath === "seasonal" && <div role="status" className={`mb-5 flex items-start gap-3 rounded-xl border p-3 text-sm ${seasonalPreparedReady ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-red-200 bg-red-50 text-red-900"}`}>
        ...
        <div>
          <div className="font-bold">{seasonalPreparedReady ? "Production render locked" : "Seasonal production render is incomplete"}</div>
          <p className="mt-1 text-xs leading-relaxed opacity-80">{seasonalPreparedReady ? "The customer mockup, 300 DPI front production file, render hash and preflight record are all present for this exact design." : "Add to cart is blocked so a missing, stale or blank seasonal render cannot become an order. Return to Seasonal Design Lab and prepare the preview again."}</p>
        </div>
      </div>}
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <ReviewCard label="Production result" value={designPath === "seasonal" ? (seasonalPreparedReady ? "Locked & preflight passed" : "Not ready") : "Customer-approved preview"} sub={designPath === "seasonal" ? (seasonalPreparedReady ? "Prepared 300 DPI layered composite · locked to this approval when added to cart." : "Return to Seasonal Design Lab and regenerate the exact preview before ordering.") : "Locked 300 DPI PNG is generated when added to cart."} />
      ...
      <button onClick={createAndAdd} disabled={addToCartDisabled} className="w-full mt-5 bg-accent text-accent-foreground py-4 font-bold uppercase tracking-wide disabled:opacity-50">{saving ? "Generating production artwork…" : seasonalPreparedReady ? "Approve, Lock & Add to Cart →" : "Production preview required"}</button>
    </div>
  );
}
