...
import { fitSeasonalArtwork, normalizeSeasonalTextStyle, seasonalSelection } from '@/lib/seasonalArtwork';
import SeasonalPersonalizationEditor, { defaultSeasonalText, normalizeSeasonalTextState, SeasonalSvgText } from '@/components/storefront/SeasonalPersonalizationEditor';
...
export function SeasonalOverlay({ artwork, layout, area, text, rotation = 0, editable = false, showSelection = true, onMove = undefined, onResize = undefined, onRotate = undefined, onEditText = undefined }) {
  const action = useRef(null);
  const tap = useRef({ at: 0 });
  const uid = useId().replace(/:/g, '');
  if (!artwork || !layout) return null;

  const nameStyle = normalizeSeasonalTextStyle(text?.nameStyle, text?.color || '#111111');
  const messageStyle = normalizeSeasonalTextStyle(text?.messageStyle, text?.color || '#111111');
  const messageLines = String(text?.message || '').split('\n').map((line) => line.trim()).filter(Boolean).slice(0, 2);
  ...
  const focusText = (target = 'name') => {
    if (!artwork.customizable) return;
    onEditText?.(target);
    document.getElementById(target === 'message' ? 'seasonal-personalization-message' : 'seasonal-personalization-name')?.focus?.();
  };
    ...
    >
      ...
      {artwork.requires_name && text?.name && (
        <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 600 600" aria-label={text.name} role="img">
          <SeasonalSvgText value={text.name} style={nameStyle} y={310} baseSize={42} idPrefix={`${uid}-name-main`} />
        </svg>
      )}
      {artwork.customizable && (text?.name || text?.message) && (
        <svg className="pointer-events-none absolute bottom-0 left-0 w-full" style={{ height: artwork.requires_name ? '18%' : '20%' }} viewBox="0 0 600 92" role="img" aria-label={[text?.name, text?.message].filter(Boolean).join(' — ')}>
          {!artwork.requires_name && <SeasonalSvgText value={text?.name} style={nameStyle} y={26} baseSize={26} idPrefix={`${uid}-name-bottom`} />}
          {messageLines.map((line, index) => <SeasonalSvgText key={`${line}-${index}`} value={line} style={messageStyle} y={messageLines.length > 1 ? 57 + index * 24 : 66} baseSize={messageLines.length > 1 ? 17 : 20} idPrefix={`${uid}-message-${index}`} />)}
        </svg>
      )}
      {editable && showSelection && <button data-control type="button" onPointerDown={(event) => beginHandle(event, 'resize')} onPointerMove={move} onPointerUp={stop} onPointerCancel={stop} className="absolute -bottom-3 -right-3 z-20 grid h-8 w-8 touch-none place-items-center rounded-full border-2 border-white bg-[#D9273E] text-white shadow-[0_5px_16px_rgba(0,0,0,.22)]" aria-label="Resize artwork"><Maximize2 size={13} /></button>}
    </div>
  );
}
...
export default function SeasonalStudio({ product, garment, color, size, variant, quantity, unitPrice, Preview, onBack, catalog: garmentCatalog = [], availableColors = [], availableSizes = [], onProductChange, onColorChange, onSizeChange, colorSwatch, priceVisibility = 'hidden', initialDraft = null, editCartKey = '' }) {
  const initialTextColor = preferredTextColor(color, colorSwatch);
  ...
  const [text, setText] = useState(() => defaultSeasonalText(initialTextColor));
  const [activeTextTarget, setActiveTextTarget] = useState('name');
  ...
  const hasText = selected?.customizable && Boolean(String(text.name || '').trim() || String(text.message || '').trim());
  ...
  const nameStyle = normalizeSeasonalTextStyle(text.nameStyle, text.color || recommendedTextColor);
  const messageStyle = normalizeSeasonalTextStyle(text.messageStyle, text.color || recommendedTextColor);
  ...
  const hasLowContrast = (target) => {
    const style = target === 'message' ? messageStyle : nameStyle;
    if (style.outlineWidth >= 2) return false;
    const exactRatio = contrastRatio(style.color, colorValue);
    if (exactRatio != null) return exactRatio < 3;
    return style.color !== recommendedTextColor;
  };
  ...
  const choose = (artwork) => {
    ...
    const preferred = preferredTextColor(color, colorSwatch);
    setSelected(artwork);
    setRequested(0);
    setPosition({ x: initial ? (area.width - initial.width) / 2 : 0, y: 0 });
    setRotation(0);
    setText(defaultSeasonalText(preferred));
  ...
  const updatePosition = (next) => {
    ...
    setPosition({ x: Math.max(0, Math.min(next.x, area.width - layout.width)), y: Math.max(0, Math.min(next.y, usableArea.height - layout.height)) });
    setApproved(false);
    setReviewMode(false);
  };
  ...
  const updateStyle = (target, patch) => {
    const styleKey = target === 'message' ? 'messageStyle' : 'nameStyle';
    setText((current) => {
      const merged = normalizeSeasonalTextStyle({ ...current[styleKey], ...patch }, current.color || recommendedTextColor);
      return { ...current, [styleKey]: merged, ...(target === 'name' && patch.color ? { color: patch.color } : {}) };
    });
    setApproved(false);
    setReviewMode(false);
  };
  ...
  useEffect(() => {
    ...
    setReviewMode(false);
    setText((current) => {
      if (String(current.name || '').trim() || String(current.message || '').trim()) return current;
      return defaultSeasonalText(preferredTextColor(color, colorSwatch));
    });
  }, [product.id, color, size]);

  useEffect(() => { saveRequestId.current = ''; }, [product.id, color, size, selected?.id, requested, position.x, position.y, rotation, text]);
  ...
  useEffect(() => {
    ...
    setText(normalizeSeasonalTextState(initialDraft.text || {}, preferredTextColor(color, colorSwatch)));
  ...
  const save = async () => {
    ...
    try {
      ...
      const renderSnapshot = { version: 2, designPath: 'seasonal', artworkId: selected.id, configuration, garment: { id: product.id, variantId: variant?.id || null, color, size } };
      ...
      const design = await customerApi.createCustomDesign({
        ...
        placement: 'front',
        photoAssets: [],
        personalization: { name: configuration.name, message: configuration.message, textStyles: configuration.text_styles },
      ...
      const seasonalSummary = {
        ...
        textStyles: configuration.text_styles,
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[linear-gradient(180deg,#F4F7FA_0%,#EDF2F6_45%,#F8FAFC_100%)] px-3 py-3 sm:px-4 sm:py-6 lg:px-8 lg:py-9">
      <div className="mx-auto min-w-0 max-w-[1580px]">
        ...
        {catalog && artworks.length === 0 && <div className="rounded-2xl border border-[#DCE3EA] bg-white p-6"><h2 className="font-semibold">Seasonal designs are being prepared</h2><p className="mt-2">New designs will appear here once approved for printing. You can continue with a photo design today.</p><button onClick={onBack} className="mt-4 underline">Choose another design option</button></div>}
