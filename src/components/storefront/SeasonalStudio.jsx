...
export function SeasonalOverlay({ artwork, layout, area, text, rotation = 0, editable = false, showSelection = true, onMove = undefined, onResize = undefined, onRotate = undefined, onDelete = undefined }) {
  ...
  return (
    <div
      ...
      onPointerDown={(event) => { if (!(event.target instanceof Element) || !event.target.closest('[data-control]')) begin(event, 'move'); }}
      onPointerMove={move}
      onPointerUp={stop}
    ...
    >
      ...
      {editable && showSelection && (
        <>
          <button data-control type="button" onClick={(event) => { event.stopPropagation(); onDelete?.(); }} className="absolute -right-4 -top-4 z-20 hidden h-8 w-8 place-items-center rounded-full bg-[#17324D] text-white shadow-lg sm:grid" aria-label="Remove artwork"><X size={14} /></button>
          <button data-control type="button" onPointerDown={(event) => begin(event, 'rotate')} className="absolute -left-4 -top-4 z-20 hidden h-8 w-8 touch-none place-items-center rounded-full bg-white text-[#17324D] shadow-lg sm:grid" aria-label="Rotate artwork"><RotateCw size={14} /></button>
          <button data-control type="button" onPointerDown={(event) => begin(event, 'resize')} className="absolute -bottom-4 -right-4 z-20 hidden h-8 w-8 touch-none place-items-center rounded-full bg-white text-[#17324D] shadow-lg sm:grid" aria-label="Resize artwork"><Maximize2 size={14} /></button>
        </>
      )}
    </div>
  );
          ...
          <div className="grid min-w-0 w-full items-start gap-4 xl:grid-cols-[minmax(300px,.78fr)_minmax(520px,1.3fr)_minmax(285px,.72fr)] xl:gap-5">
            ...
            <section ref={previewSectionRef} aria-label="Garment preview" className="order-2 min-w-0 w-full scroll-mt-20 xl:sticky xl:top-4">
              ...
              <div className="overflow-hidden rounded-3xl border border-[#CDD7E0] bg-white p-3 shadow-[0_24px_60px_rgba(23,50,77,.12)]">
                ...
                <p className="px-2 pb-1 pt-3 text-center text-xs text-[#61717F]"><Move size={12} className="mr-1 inline" /> Drag artwork on the garment. The preview stays visible while you browse or edit on desktop.</p>
              </div>
            </section>
