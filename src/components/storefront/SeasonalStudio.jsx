...
export function SeasonalOverlay({ artwork, layout, area, text, rotation = 0, editable = false, showSelection = true, onMove = undefined, onResize = undefined, onRotate = undefined, onDelete = undefined }) {
  ...
  return <div data-seasonal-artwork="true" onPointerDown={e=>{if(!(e.target instanceof Element) || !e.target.closest('[data-control]'))begin(e,'move');}} onPointerMove={move} onPointerUp={stop} onPointerCancel={stop}
    className={"absolute touch-none select-none " + (editable?'cursor-move ':'pointer-events-none ') + (editable&&showSelection?'ring-2 ring-white shadow-[0_0_0_1px_rgba(23,50,77,.8)]':'')}
    ...
    {editable && showSelection && <>
      <button data-control type="button" onClick={e=>{e.stopPropagation();onDelete?.();}} className="absolute -right-4 -top-4 z-20 hidden h-8 w-8 place-items-center rounded-full bg-[#17324D] text-white shadow-lg sm:grid" aria-label="Remove artwork"><X size={14}/></button>
      <button data-control type="button" onPointerDown={e=>begin(e,'rotate')} className="absolute -left-4 -top-4 z-20 hidden h-8 w-8 touch-none place-items-center rounded-full bg-white text-[#17324D] shadow-lg sm:grid" aria-label="Rotate artwork"><RotateCw size={14}/></button>
      <button data-control type="button" onPointerDown={e=>begin(e,'resize')} className="absolute -bottom-4 -right-4 z-20 hidden h-8 w-8 touch-none place-items-center rounded-full bg-white text-[#17324D] shadow-lg sm:grid" aria-label="Resize artwork"><Maximize2 size={14}/></button>
    </>}
  </div>;
}
...
export default function SeasonalStudio({ product, garment, color, size, variant, quantity, unitPrice, Preview, onBack, catalog: garmentCatalog = [], availableColors = [], availableSizes = [], onProductChange, onColorChange, onSizeChange, colorSwatch, priceVisibility = 'hidden', initialDraft = null, editCartKey = '' }) {
  ...
  const choose = a => {const initial=fitSeasonalArtwork(a,area,0);setSelected(a);setRequested(0);setPosition({x:initial?(area.width-initial.width)/2:0,y:0});setRotation(0);setText({name:'',message:'',color:preferredTextColor(color,colorSwatch)});setApproved(false);setReviewMode(false);setError('');window.setTimeout(()=>previewSectionRef.current?.scrollIntoView({behavior:'smooth',block:'start'}),50);};
  const updatePosition = next => { if (!layout || !usableArea) return; setPosition({x:Math.max(0,Math.min(next.x,area.width-layout.width)),y:Math.max(0,Math.min(next.y,usableArea.height-layout.height))});setApproved(false);setReviewMode(false); };
  ...
  return <main className="min-h-screen bg-[linear-gradient(180deg,#F4F7FA_0%,#EDF2F6_45%,#F8FAFC_100%)] px-3 py-3 sm:px-4 sm:py-6 lg:px-8 lg:py-9">
    ...
    {artworks.length>0 && !reviewMode && <div className="grid items-start gap-4 xl:grid-cols-[minmax(300px,0.78fr)_minmax(520px,1.3fr)_minmax(285px,0.72fr)] xl:gap-5">
      <section aria-label="Choose seasonal artwork" className="order-3 rounded-3xl border border-[#DCE3EA] bg-white/85 p-4 shadow-[0_14px_40px_rgba(23,50,77,.07)] lg:p-5 xl:order-1">
      ...
      <section aria-label="Garment and artwork controls" className="order-2 space-y-4 xl:order-3">
