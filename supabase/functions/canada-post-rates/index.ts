...
function respond(req: Request, body: unknown, status = 200) { return new Response(JSON.stringify(body), { status, headers: headers(req) }); }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return respond(req, { configured: true, available: rates.length > 0, rates, fallbackRecommended: rates.length === 0, source: "canada_post" });
  } catch (error) {
