...
editor = replace_once(editor, '  onResetAll,\n', '  onResetCurrentSide,\n  onResetEntireDesign,\n', "editor reset props")
editor = replace_once(
    editor,
...
# Radix forwardRef components need explicit JSDoc parameter types under checkJs.
# This preserves runtime/styling and fixes the shared notification components' public prop inference.
for ui_path, names in [
    (Path("src/components/ui/alert-dialog.jsx"), ["AlertDialogOverlay", "AlertDialogContent", "AlertDialogTitle", "AlertDialogDescription", "AlertDialogAction", "AlertDialogCancel"]),
    (Path("src/components/ui/toast.jsx"), ["ToastViewport", "Toast", "ToastAction", "ToastClose", "ToastTitle", "ToastDescription"]),
]:
    source = ui_path.read_text()
    for name in names:
        marker = f"const {name} = React.forwardRef(({{ className"
        replacement = f"const {name} = React.forwardRef((/** @type {{any}} */ {{ className"
        if marker not in source:
            raise RuntimeError(f"forwardRef typing bridge: missing {name}")
        source = source.replace(marker, replacement, 1)
    ui_path.write_text(source)

print("Touch Studio reset controls and notification typing bridge applied.")
