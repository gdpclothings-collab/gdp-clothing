...
def replace_once(source: str, before: str, after: str, label: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f'Patch target is not unique ({count} matches): {label}')
...
editor = replace_once(
    ...
    bootleg_photo_helpers + 'function StickerLayer({ layer, selected, sticker, canvasRef, onSelect, onTransform }) {',
...
old_patch_active_photo = r'''  const patchActivePhotoTransform = (patch) => {
...
'''
new_patch_active_photo = r'''  const patchActivePhotoTransform = (patch) => {
    if (!activePhoto) return;
    if (isBootleg) setActiveBootlegLayer('photo');
...
'''
editor = replace_once(editor, old_patch_active_photo, new_patch_active_photo, 'persist free-canvas photo coordinates')

old_duplicate = "    const duplicate = { ...activePhoto, id: v2LayerId('photo'), transform: { ...activePhoto.transform, x: clamp(Number(activePhoto.transform?.x || 0) + 8, -48, 48), y: clamp(Number(activePhoto.transform?.y || 0) + 8, -48, 48) } };\n"
new_duplicate = r'''    const duplicatePosition = resolvePhotoCanvasPosition(activePhoto.transform || {}, template?.photoZone || { x: 15, y: 12, width: 70, height: 68 });
...
'''
editor = replace_once(editor, old_duplicate, new_duplicate, 'duplicate free-canvas photo position')

old_photo_zone = r'''  const photoZone = (
...
'''
new_photo_zone = r'''  const photoZone = (
    <div ref={zoneRef} className="absolute overflow-hidden" style={{ left: `${zone.x}%`, top: `${zone.y}%`, width: `${zone.width}%`, height: `${zone.height}%`, borderRadius: radius }}>
...
'''
editor = replace_once(editor, old_photo_zone, new_photo_zone, 'add full-print-area Photo Bootleg canvas')

old_linked_render = r'''          {isBootleg ? (
...
old_move_controls = r'''        <RangeControl label="Move left / right" value={activePhoto.transform?.x || 0} min={-48} max={48} suffix="%" onChange={(value) => patchActivePhotoTransform({ x: value })} />
...
'''
editor = replace_once(editor, old_move_controls, new_move_controls, 'use full print-area photo movement sliders')

old_reset = r'''        <button type="button" onClick={() => patchActivePhotoTransform({ scale: template?.defaultTransform?.scale || 100, rotation: 0, x: 0, y: 0 })} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-black text-slate-700"><RotateCcw size={14} /> Reset selected photo</button>
'''
new_reset = r'''        <button type="button" onClick={() => {
...
'''
editor = replace_once(editor, old_reset, new_reset, 'reset Bootleg photo to its print-area start position')
...
production = replace_once(
    ...
    production_helpers + 'function withTemplateTransform(context, widthPx, heightPx, transform = {}, draw) {',
    'insert free-canvas production photo renderer',
)

old_draw_photos = r'''  const drawPhotos = async () => {
...
'''
new_draw_photos = r'''  const drawPhotos = async () => {
    for (const layer of photos) {
      const loaded = await loadImage(layer.asset?.url, 'A customer photo layer could not be reopened.');
      try {
...
'''
production = replace_once(production, old_draw_photos, new_draw_photos, 'render saved free-canvas photos at 300 DPI')
...
for token in required_editor_tokens:
    if token not in editor:
        raise RuntimeError(f'Editor verification failed: {token}')
if 'function drawFreePhoto' not in production or 'hasFreePhotoCanvasTransform' not in production:
    raise RuntimeError('Production verification failed: free-canvas renderer was not installed.')

EDITOR_PATH.write_text(editor, encoding='utf-8')
PRODUCTION_PATH.write_text(production, encoding='utf-8')
print('Applied full-print-area Photo Bootleg photo movement without changing Memorial photo-zone behavior.')
